...
class MyIterableDataset(torch.utils.data.IterableDataset):
    ...
    def __iter__(self):
         ...
         return iter([torch.zeros([TENSOR_SIZE], dtype=torch.float32) for i in range(self.iter_start, self.iter_end)])
...
def train(gpu, args):
    ...
    for epoch in range(args.epochs):
        for i, images in enumerate(train_loader):
            ...
            labels = torch.zeros([100], dtype=torch.long).cuda(non_blocking=True) #labels.cuda(non_blocking=True)
            # Forward pass
            ...
            torch.distributed.sgd_update(weight, grad, alpha)
            print(weight)
